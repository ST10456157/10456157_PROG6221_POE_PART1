...
");
            ...
            while (true)
            {
                ...
                if (string.IsNullOrWhiteSpace(input))// adds text coloring and formatting
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                ...
                if (input.ToLower() == "exit") // If user types exit, console shuts down
                {
        ...
        static void RespondToUser(string input)// Created to showcase the example questions posed and the answers thereafter using a switch statement. And if the question is invalid it can handle it an respond by asking for new question
        {
            switch (input)
            {
                case "how are you?":
